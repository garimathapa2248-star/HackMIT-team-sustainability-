...
- chore(repo): the package now lives in the `cloud-native` monorepo (`packages/carto-api-client`) and is published from there by `api-client-release.yml`; the published artifact keeps the same entry points, exports and dependency ranges as 0.5.33
- feat(widgetSources): `widgetWorkerType` option to run `widgetWorkerUrl` as a `module` worker (default stays `classic`)
- chore(deps): bump the bundled turf group to 7.4.0. `@turf/boolean-contains` now checks edge midpoints as well as vertices, so spatial filters on concave polygons no longer accept cells that leave the polygon between vertices
- chore(deps): apache-arrow re-resolved to 21.2.0 so every bundled dependency is ESM again (`json-bignum` leaves the graph)
