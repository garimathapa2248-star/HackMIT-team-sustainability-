...
import { markdownDoc, pdfUrl } from "./api";
import { useDash } from "./context";
import { money } from "./format";
import { SAVED_BUDGET, siteNoun } from "./plan";
...
export default function ExportPlanForm() {
  const d = useDash();
  const { plan, budget, mode, loading, city, cityName } = d;
  const sites = siteNoun(plan, d.candidates);
  const locked = !d.liveSolve || loading;
  ...
  // The API renders the PDF from whatever plan.json currently holds, so it always matches.
  const pdfOk = true;
  useEffect(() => {
          ...
          <div className="form-field" role="group" aria-labelledby="f-budget">
            ...
            <p className="help small">
              {d.liveSolve
                ...
                : `Locked to the saved ${money(SAVED_BUDGET)} plan.`}
            </p>

            {/* Re-solving rewrites plan.json and three other artifacts on the server, so it is
                off by default and asks before the first run. */}
            <label className="solve-toggle">
              <input
                type="checkbox"
                checked={d.liveSolve}
                onChange={(event) => {
                  if (!event.target.checked) return d.setLiveSolve(false);
                  const ok = window.confirm(
                    "Run a new solve?\n\nThis recalculates the portfolio on the server and " +
                      "overwrites the saved plan, the before/after risk layers and the flood " +
                      "counterfactual. The frozen $2M result on this site will be replaced.\n\n" +
                      "Leave this off for a demo."
                  );
                  if (ok) d.setLiveSolve(true);
                }}
              />
              <span>
                <b>Run a new solve on the server</b>
                <small>
                  Off by default. Turning this on lets the budget and objective recalculate — and
                  overwrites the saved plan.
                </small>
              </span>
            </label>
          </div>
            ...
            <div className="radio-cards">
              ...
              <label className={`radio-card ${format === "pdf" ? "on" : ""} ${pdfOk ? "" : "off"}`}>
                ...
                <span className="rc-sub">
                  {pdfOk
                    ? `Rendered by the API from the current plan.`
