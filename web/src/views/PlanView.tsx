...
import PlanDeepDive, { ResidualRisk } from "../PlanDeepDive";
import MapPanel from "../MapPanel";
import PlanTiles from "../PlanTiles";
import { earlyBenefitOf, selectedTypes, siteNoun } from "../plan";
import { Accordion, Card, InfoPopover } from "../ui";
...
function OptimizerStatus() {
  const { optimizerState, liveSolve, plan } = useDash();
  const state = liveSolve ? optimizerState : optimizerState === "unavailable" ? "unavailable" : "frozen";
  const text = {
    live: {
      label: "Live optimizer",
      body: "Budget and objective changes recalculate the portfolio through the API.",
    },
    checking: { label: "Loading the plan…", body: "Reading the saved plan from the API." },
    frozen: {
      label: "Saved plan",
      body: `The ${plan ? money(plan.budget_usd) : ""} plan as solved and saved. Moving the budget shows other budgets the optimizer already solved, and changes nothing on the server.`,
    },
    unavailable: {
      label: "Plan unavailable",
      body: "The API returned no plan for this region, so no control can imply a result that was not computed.",
    },
  }[state];
  return (
...
function BudgetCard() {
  const d = useDash();
  const { plan, budget, mode, loading } = d;
  const sites = siteNoun(plan, d.candidates);

  // The frontier is a set of budgets the optimizer already solved. Stepping between them is
  // instant, uses real numbers, and never POSTs /optimize (which would overwrite the saved plan).
  const steps = useMemo(
    () => [...(plan?.frontier || [])].sort((l, r) => l.budget_usd - r.budget_usd),
    [plan]
  );
  const index = steps.length
    ? steps.reduce(
        (best, point, i) =>
          Math.abs(point.budget_usd - budget) < Math.abs(steps[best].budget_usd - budget) ? i : best,
        0
      )
    : 0;
  const at = steps[index];
  const locked = !steps.length || loading;
  ...
  return (
    <div className="hero-stat plan-card">
      ...
      <div className="plan-budget">{money(at?.budget_usd ?? budget)}</div>
      <small className="plan-sub">
        {at?.n_selected != null ? `${at.n_selected} ${sites}` : plan ? `${plan.selected.length} ${sites}` : "—"}
        {at && ` · ${risk(at.people_protected)} annual people-risk avoided`}
      </small>
      ...
      <div>
        <input
          ...
          disabled={locked}
          onChange={(event) => d.setBudget(steps[Number(event.target.value)].budget_usd)}
        />
        <div className="slider-scale">
          <span>{steps.length ? money(steps[0].budget_usd) : "—"}</span>
          <span>{steps.length ? money(steps[steps.length - 1].budget_usd) : "—"}</span>
        </div>
      </div>
      ...
      {at && (
        <dl className="budget-facts">
          <div>
            <dt>Carbon over 10 years</dt>
            <dd>{metric(at.co2_t_10yr, 0)} t</dd>
          </div>
          <div>
            <dt>Local income a year</dt>
            <dd>{at.income_usd_yr == null ? "—" : money(at.income_usd_yr)}</dd>
          </div>
          <div>
            <dt>Benefit per dollar</dt>
            <dd>{at.bcr == null ? "—" : `${metric(at.bcr, 0)}×`}</dd>
          </div>
        </dl>
      )}

      <small className="shift-note">
        Every step here is a budget the optimizer already solved. The map, mix and site table below
        stay on the saved {plan ? money(plan.budget_usd) : ""} {mode} plan.
      </small>
    </div>
  );
}
