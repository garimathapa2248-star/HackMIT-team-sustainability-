...
export default function AskView() {
  ...
  const [source, ...rest] = d.answerMeta.split(" · ");
      ...
      <section className="ask-panel" id="ask-composer" aria-label="Ask a question">
        ...
        <div className="answer-card" aria-live="polite">
          <div className="answer-head">
            ...
            {d.answerMeta && !d.asking && <Badge tone="accent">{source}</Badge>}
          </div>
          {d.asking ? (
          ...
          {d.answerMeta && !d.asking && rest.length > 0 && (
            <p className="answer-source">Sources: {rest.join(" · ")}</p>
          )}
        </div>
      </section>
