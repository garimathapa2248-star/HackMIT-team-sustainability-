...
import { Counts, CrossValidation, ScenarioLedger, Strategies } from "../ProofExtras";
...
export default function ProofView() {
  ...
  const takeaway = !hasFlood
    ...
    : `The flood map is a rough guide to where flooding is likely, not a street-level prediction. It scored ${cal.critical_success_index?.toFixed(
        ...
        val?.critical_success_index != null
          ? `, and ${val.critical_success_index.toFixed(3)} on the ${y2} flood it had never seen`
          : ""
      }. ${
        allLower
  ...
  return (
    <div className="view">
              ...
              <Card title="Against simple baselines" subtitle="Match score out of 1 · higher is better">
                ...
                {jrcCsi != null && (
                  <Callout tone={beats ? "accent" : "warn"}>
                    {beats
                      ? `Beats a simple published water map: ${modelCsi.toFixed(3)} against ${jrcCsi.toFixed(3)}.`
                      : `A simple published water map scores higher on this flood (${jrcCsi.toFixed(
                          3
                        )} against our ${modelCsi?.toFixed(3)}). We report the miss.`}
                  </Callout>
                )}
              </Card>
            )}
              ...
              >
                ...
                <div className="chart">
                  <ResponsiveContainer>
                    <ComposedChart data={skillRows} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
                      ...
                      <Tooltip content={<ChartTooltip title={(l) => `${l} km squares`} fmt={(v) => v.toFixed(3)} />} />
                ...
                {firstCsi != null && lastCsi != null && (
                  <p className="chart-note">
                    The match score {lastCsi < firstCsi ? "falls" : "rises"} as the squares get bigger ({firstCsi.toFixed(3)} →{" "}
                    {lastCsi.toFixed(3)}).
                  </p>
                )}
              </Card>
      ...
      <div className="section-title">
        <h2>How hard we tested it</h2>
        <p>
          The score above was fitted on the flood it grades. These are the checks that were not.
        </p>
      </div>
      <Counts />
      <CrossValidation />
        ...
        <Accordion title="Fine print on the proof">
          ...
          {cal.spatial_holdout?.critical_success_index != null && (
            <p className="muted-p">
              ...
              {cal.spatial_holdout.critical_success_index.toFixed(3)}. The same storm split spatially, not a second event.
            </p>
          )}
