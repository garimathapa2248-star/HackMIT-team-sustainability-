...
// Acronyms the backend writes lower-case inside snake_case ids.
const ACRONYMS: Record<string, string> = { glof: "GLOF", om: "O&M", co2: "CO\u2082", npv: "NPV", bcr: "BCR" };

/** Falls back to sentence case for measure types the catalogue adds later. */
export const typeLabel = (type: string) => {
  if (TYPE_LABEL[type]) return TYPE_LABEL[type];
  const words = type.split("_").map((w) => ACRONYMS[w] || w);
  const first = words[0];
